...
| 退出 | 先弹退出确认框（含"同时关闭当前 dsh 服务"勾选，勾选即时记忆）；默认不勾 = dsh 放行继续运行；勾选时先优雅停止 dsh（最多等 8 秒）再退出托盘 |
...
使用固定的 Python 解释器和 PyInstaller（由 `build.bat` 顶部的 `PY` 变量指定）：

```text
build.bat nopause
```

`build.bat` 会先跑编译门禁，再把 `onedir/noconsole` 包构建到 `release\dsh-helper-<版本>\`，然后运行 `--smoke` 验证 dsh 命令与端口决策（固定端口可用则用固定端口，否则回退自动端口）。每个版本一个独立目录；目录已存在即拒绝构建，保证每版产物可复现。
...
- **退出时 dsh 怎么办**：点击"退出"会先弹确认框，带"同时关闭当前 dsh 服务"勾选项。勾选状态在点下的一瞬即持久化（默认不勾——dsh 在托盘退出后继续运行，这是正常的健康状态，托盘下次启动会自动重新接管）。勾选时 dsh 才会先优雅退出（等待最多 8 秒），完成插件清理和会话落盘，只有超时才强制结束进程。这 8 秒里菜单第一行显示"状态：停止中"、图标转灰，且"启动 / 停止 / 重启 / 退出"全部置灰，重复点击无效——这是刻意的，请等它自己消失。
